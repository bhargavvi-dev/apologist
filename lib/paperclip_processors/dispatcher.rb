  ...
  class ImageMagick < BaseProcessor
    ...
    def make
      ...
      if resource_spec.name == 'OFFICE_TEXT_LOGO'
        ...
        font_file = "#{Rails.root}/app/assets/fonts/hinted-HelveticaNeue-Medium.ttf"
        text = @resource.resource_holder.logo_text
        command = "-i #{file(@file)} -vf 'drawtext=fontfile=#{font_file}: \ text=#{text}: fontcolor=blue: fontsize=24: x=w-tw-5:y=h-th-8' -y #{file(temp)}"
        ...
        if coordinate and coordinate != '0'
          log 'imagemagick processor: crop image...'
          ...
          log 'imagemagick processor: without crop image...'
          temp = temp_file @basename, @current_format
